...
def get_aligned_waiting_times(bus_times, expected_waittimes):
    ...
    for i, _ in enumerate(bus_times):
        print(
            f'If bus {bus_times[i]} arrives {modulos[i]} minutes before n,'
            f'the bus will also be arriving at {expected_waittimes[i]}'
        )
    ...
    for i, bt in enumerate(bus_times[1:]):
        ...
        print(s[0])
        print(type(solution))
        solution = (solution * s[2] * bt + int(modulos[i+1]) * s[1] * prod) % new_prod
        prod = new_prod
    return solution + 1 # Dunno why I systematically underestimate the solution by 1
